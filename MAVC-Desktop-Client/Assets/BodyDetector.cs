    ...
    void Update()
    {
        ...
        foreach (var interactionSourceState in interactionSourceStates)
        {
            if (interactionSourceState.source.handedness == InteractionSourceHandedness.Left)
            {
                ...
                if (interactionSourceState.thumbstickPosition.y < -0.5)
                {
                    playWalkingBackwardAnimation = true;
                }
                else if (interactionSourceState.thumbstickPosition.y > 0.5 || Mathf.Abs(interactionSourceState.thumbstickPosition.x) > 0.5)
                {
